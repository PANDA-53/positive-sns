    ...
    <main className="min-h-screen bg-[#F2F2F2] text-black pb-12 font-sans">
      ...
      <nav className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-gray-200 mb-6">
        <div className="max-w-2xl mx-auto px-4 h-16 flex justify-between items-center">
          <h1 className="text-lg md:text-xl font-bold text-black tracking-tight underline decoration-gray-300 underline-offset-4">Timeline</h1>
          {user ? (
            <form action={logout}>
      ...
      <div className="max-w-2xl mx-auto px-4 space-y-8">
        ...
        <div className="space-y-6">
          {mainPosts?.map((post) => (
            <div key={post.id} className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-5 md:p-7 hover:border-gray-300 transition-colors">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex flex-col">
                  <span className="text-sm md:text-base font-bold text-gray-900 leading-none">{post.user_name || '匿名'}</span>
                ...
                <div className="ml-4 md:ml-8 space-y-4 border-l-2 border-gray-100 pl-4 md:pl-6 mb-6">
                  {replies?.filter(r => r.parent_id === post.id).map(reply => (
                    <div key={reply.id} className="text-sm md:text-base bg-gray-50 p-3 rounded-2xl">
                      <span className="font-bold text-gray-600 block mb-1 text-xs">{reply.user_name || '匿名'}</span>
              ...
              {/* 返信フォーム：グレースケール */}
              {user && (
                <form action={createReply} className="flex items-center gap-2 mt-4 bg-gray-50 p-2 rounded-full border border-gray-100 focus-within:border-gray-300 focus-within:bg-white transition-all">
