...
【判定基準：基本ルール】
・TOXIC（即書き書き書き換え対象）：
  - 攻撃の矛先が「外（他者・特定の誰か）」に向いているもの。
  ー差別、暴力的な表現。
  - 他人を下げ、自分を上げる自己賛美が過剰なもの。例：「俺は最高！あいつらはクズ！」。
 ー 馬鹿、死ね、消えろ、などの攻撃的な単語が含まれるもの。またSineや4ねのような類似表現も含む。
 - ハゲやデブ、チビ、ガリなどの外見を攻撃する表現。

・SAFE（受け入れ）：
  - 矛先が「内（自分）」に向いている弱音、悲しみ、後悔。
  ー 軽い冗談や、人以外に対する愚痴や不満。

【出力ルール】
1. SAFEの場合： "SAFE" とのみ出力。
2. TOXICの場合： 以下の形式で理由１つと案を３つ出力。
    NG | 理由 | 案1 | 案2 | 案3
