...
export default async function Index() {
  ...
  if (postsError) console.error('Post Fetch Error:', postsError.message)
  ...
  return (
    <main className="max-w-xl mx-auto p-4 md:p-6 min-h-screen bg-gray-50 text-black font-sans">
      <header className="flex justify-between items-center mb-8 bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
        ...
        {user && (
          <form action={logout}>
            <button className="text-[10px] bg-red-50 text-red-500 px-4 py-2 rounded-full font-bold hover:bg-red-100 transition-colors">
              ログアウト
            </button>
          </form>
        )}
      ...
      {user ? (
        <section className="mb-10 space-y-4">
          ...
          <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4">
            <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-green-100 bg-gray-100 shrink-0 shadow-inner">
              <img 
            ...
            <form action={uploadAvatar} className="flex-1 space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="submit" className="bg-green-600 text-white text-[10px] px-3 py-1.5 rounded-full font-bold shadow-sm hover:bg-green-700">
                  画像更新
                </button>
              </div>
            </form>
          ...
          <form action={createPost} className="bg-white p-6 rounded-3xl shadow-lg border border-green-50">
            <textarea 
              ...
              className="w-full p-4 bg-gray-50 rounded-2xl text-black resize-none outline-none focus:ring-2 focus:ring-green-400 border-none" 
              rows={3} 
              required 
            />
            <button type="submit" className="mt-3 w-full bg-green-500 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-green-600">
              ポジティブをシェア
            </button>
          </form>
        </section>
        ...
        <div className="mb-10 p-10 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
          <a href="/login" className="inline-block bg-green-500 text-white px-10 py-3 rounded-full font-bold shadow-lg">
            ログイン画面へ
          </a>
        </div>
      )}
      ...
      <div className="space-y-6">
        {posts?.map((post: any) => (
          <div key={post.id} className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-full overflow-hidden border border-gray-50 bg-gray-100">
                  <img 
