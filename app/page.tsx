    ...
    <main className="max-w-xl mx-auto p-4 md:p-6 min-h-screen bg-gray-50 text-black">
      ...
      {user ? (
        <section className="mb-10 space-y-4">
          {/* プロフィールエリア：URLが不正でも落ちないように onError を設定 */}
          ...
          <form action={createPost} className="bg-white p-6 rounded-3xl shadow-lg border border-green-50">
            <textarea name="content" placeholder="最近あった、いいことは？" className="w-full p-4 bg-gray-50 rounded-2xl text-black resize-none outline-none focus:ring-2 focus:ring-green-400 border-none" rows={3} required />
        ...
        <div className="mb-10 p-10 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
          <a href="/login" className="inline-block bg-green-500 text-white px-10 py-3 rounded-full font-bold shadow-lg">ログインして参加</a>
        </div>
      )}
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-full overflow-hidden border bg-gray-100">
                  <img 
                    src={post.avatar_url && post.avatar_url.startsWith('http') ? post.avatar_url : "https://www.gravatar.com/avatar/?d=mp"} 
                    className="w-full h-full object-cover" 
                  />
                </div>
