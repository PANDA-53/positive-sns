    ...
    <main className="min-h-screen bg-[#F2F2F2] text-black pb-12 font-sans">
      <nav className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-gray-200 mb-6">
        <div className="max-w-2xl mx-auto px-4 h-16 flex justify-between items-center">
          <h1 className="text-lg md:text-xl font-bold text-black tracking-tighter italic">Timeline.</h1>
          {user && (
            <form action={logout}>
              <button className="text-[10px] md:text-xs bg-white border border-gray-200 text-gray-500 px-4 py-2 rounded-full font-bold hover:bg-gray-50 transition-colors">
                ログアウト
              </button>
            </form>
          )}
            ...
            <div className="space-y-6">
              {mainPosts.map((post) => (
                <div key={post.id} className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-5 md:p-7">
                  ...
                  <p className="text-base md:text-lg text-gray-800 leading-relaxed font-medium mb-6">{post.content}</p>

                  {/* 返信一覧 */}
                  {replies.filter(r => r.parent_id === post.id).length > 0 && (
